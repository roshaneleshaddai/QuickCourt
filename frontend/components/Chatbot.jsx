    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3 hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex items-center text-yellow-500 text-xs">
          ...
          <span className="text-gray-500 ml-1">({venue.ratingCount || 0})</span>
        </div>
      </div>
      ...
      {venue.priceRange && venue.priceRange.average > 0 && (
        <div className="flex items-center text-green-600 text-xs mb-2">
          ...
          <span>₹{venue.priceRange.min}-{venue.priceRange.max}/hr (Avg: ₹{venue.priceRange.average})</span>
        </div>
      )}

      {venue.courts && venue.courts.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-gray-600 mb-1">Available Courts ({venue.courts.length}):</p>
          <div className="space-y-1">
            {venue.courts.slice(0, 2).map((court, index) => (
              <div key={index} className="flex justify-between items-center text-xs">
                <span className="text-gray-700">
                  {court.name} 
                  <span className="text-blue-600 ml-1">({court.sport})</span>
                </span>
            ...
            {venue.courts.length > 2 && (
              <p className="text-xs text-gray-500">+{venue.courts.length - 2} more courts</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2">
        <button 
          ...
          className="flex-1 bg-blue-600 text-white text-xs py-2 px-2 rounded hover:bg-blue-700 transition-colors"
        >
          View Details
        </button>
        <button 
          onClick={() => window.open(venue.bookingUrl || `/book/${venue.id}`, '_blank')}
          className="flex-1 bg-gradient-to-r from-green-600 to-green-500 text-white text-xs py-2 px-2 rounded hover:from-green-700 hover:to-green-600 transition-all shadow-sm"
        >
          Book Now
        </button>
      </div>
        ...
        <div className={`p-3 rounded-lg ${
          ...
          {message.venueButtons && message.venueButtons.length > 0 && (
            <div className="mt-3 space-y-2">
              <p className="text-xs text-gray-600 font-medium">🏟️ Available Venues:</p>
              {message.venueButtons.map((venue, index) => (
                <button
                  key={index}
                  onClick={() => handleQuickAction(venue)}
                  className="w-full text-left bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 hover:border-green-400 p-3 rounded-lg transition-all hover:shadow-md text-sm"
                >
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 z-40 flex items-center justify-center ${
